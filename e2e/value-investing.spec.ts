...
test.describe('Value Intelligence Platform page', () => {
  ...
  test('column sorting reorders the table', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Sort by D/E' }).click();
    const firstAsc = await screen.locator('tbody tr').first().textContent();
    await page.getByRole('button', { name: 'Sort by D/E' }).click();
    const firstDesc = await screen.locator('tbody tr').first().textContent();
    expect(firstAsc).not.toBe(firstDesc);
    // Descending D/E must surface the most levered name, ORCL.
    expect(firstDesc).toContain('ORCL');
  });
  ...
  test('methodology section discloses factors, weights, and limitations', async ({ page }) => {
    ...
    await expect(methodology).toContainText('not investment advice');
  });
  ...
  test('page is honest: no fake live-data indicators remain', async ({ page }) => {
    ...
    for (const banned of ['LIVE_MODE', 'ACTIVE_FETCH', 'LAST_SYNC', 'SEC_FILINGS']) {
