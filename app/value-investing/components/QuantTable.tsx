...
const COLUMNS: { key: SortKey; label: string; title: string }[] = [
  ...
  { key: 'grahamNumber', label: 'Graham No.', title: 'sqrt(22.5 x EPS x book value per share), per share' },
    ...
    <div className='bg-background dark:bg-card/50 border border-border rounded-2xl overflow-hidden'>
      <div className='p-6 border-b border-border flex flex-wrap items-center justify-between gap-3'>
        ...
        <button
          ...
          className='text-[10px] font-mono px-3 py-1.5 rounded-md bg-primary/10 text-cyan-500 border border-cyan-500/20 hover:bg-primary/20 transition-colors flex items-center gap-1.5'
        >
          ...
          <thead>
            <tr className='bg-slate-50/50 dark:bg-background/50'>
              {COLUMNS.map(({ key, label, title }) => (
                <th key={key} className='px-5 py-4 text-[10px] font-mono text-muted-foreground uppercase tracking-wider border-b border-border'>
                  <button
                    onClick={() => handleSort(key)}
                    title={title}
                    aria-label={`Sort by ${label}`}
                    className={clsx('flex items-center gap-1.5 uppercase hover:text-primary transition-colors', key === sortKey && 'text-primary')}
                  >
                    {label}
                    {key === sortKey ? (
              ...
              <th className='px-5 py-4 text-[10px] font-mono text-muted-foreground uppercase tracking-wider border-b border-border'>Signal</th>
            </tr>
          </thead>
          <tbody className='divide-y divide-slate-200 dark:divide-slate-800'>
            {sorted.map((row) => (
              <tr key={row.ticker} className='hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors group'>
                ...
                <td className='px-5 py-4 text-xs font-mono font-bold text-emerald-500'>{row.roePct.toFixed(1)}%</td>
                <td className='px-5 py-4 text-xs font-mono text-muted-foreground'>{row.operatingMarginPct.toFixed(1)}%</td>
                <td className='px-5 py-4 text-xs font-mono text-muted-foreground'>{row.fcfMarginPct.toFixed(1)}%</td>
                <td className='px-5 py-4 text-xs font-mono text-muted-foreground'>{row.revenueCagrPct.toFixed(1)}%</td>
                <td className={clsx('px-5 py-4 text-xs font-mono font-bold', row.debtToEquity < 0.5 ? 'text-cyan-500' : 'text-muted-foreground')}>
                  {row.debtToEquity.toFixed(2)}
                </td>
                <td className='px-5 py-4 text-xs font-mono text-muted-foreground'>{row.grahamNumber.toFixed(1)}</td>
                <td className='px-5 py-4'>
                ...
                <td className='px-5 py-4'>
                  <div className='flex items-center gap-2'>
                    ...
                    <span
                      className={clsx(
                        'text-xs font-bold',
                        row.signal === 'Exceptional' ? 'text-emerald-500' : row.signal === 'Strong' ? 'text-cyan-500' : row.signal === 'Solid' ? 'text-amber-500' : 'text-muted-foreground',
                      )}
                    >
                      {row.signal}
                    </span>
